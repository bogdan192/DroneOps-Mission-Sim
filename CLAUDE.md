...
- Read-only ATAK drone connector registration, heartbeat, and middleware
  scaffolding in `integration_contracts/drone_connectors.py`.
- Simulated ATAK-equipped drone, personnel/team, and air-asset tracks in
  `sim_adapters/atak_feeds.py`.
- Simulated ATAK drone connector records in `sim_adapters/drone_connectors.py`.
...
docs/ATAK_DRONE_CONNECTORS.md     Read-only ATAK drone connector scaffold
...
integration_contracts/drone_connectors.py Read-only connector lifecycle records
